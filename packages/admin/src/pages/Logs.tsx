...
export function Logs() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        // Logs API endpoint not yet implemented on server.
        // When it is, fetch logs here.
      } catch {
        setError("Cannot connect to DarshJDB server. Is the server running?");
      } finally {
        setLoading(false);
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* Error state */}
      {error && (
        <div className="flex items-center gap-2 px-6 py-3 bg-red-500/10 border-b border-red-500/20 text-red-400 text-xs">
