...
mod tests {
    ...
    //
    // ddb.query end-to-end coverage against SqliteStore lives in
    // `packages/server/tests/sqlite_e2e_query.rs` (6 integration
    // tests). The v0.3.2 placeholder unit test
    // `ddb_query_with_context_returns_invalid_query_from_sqlite`
    // was removed in v0.3.3 once the executor rewire shipped and
    // SqliteStore::query started returning real rows — its assertion
    ...
    // ── ddb.kv.* wired host API tests (v0.3.3) ────────────────────
    //
    // These tests exercise the live-context path of `install_ddb_api`
