# DarshanDB vs Supabase: Architectural Comparison

A technically honest comparison between DarshanDB and Supabase, written for engineers evaluating both systems. Neither product is universally better -- each makes fundamental tradeoffs that favor different use cases.

**Versions compared**: DarshanDB 0.1.0 (single-binary Rust server) vs Supabase (managed platform / self-hosted Docker Compose).
...
11. [What DarshanDB Does Better](#11-what-darshandb-does-better)
12. [Migration Path](#12-migration-path-supabase-to-darshandb)
...
### DarshanDB: Monolithic Binary

DarshanDB compiles to a single Rust binary that embeds all subsystems:

```
darshandb (single process)
...
| **Resource consumption** | 2-4 GB RAM minimum for the full stack at rest | 30-80 MB RSS typical for a loaded DarshanDB instance |
...
**Verdict**: Supabase's microservice split is the right architecture for a managed cloud platform that needs to scale individual bottlenecks. DarshanDB's monolith is the right architecture for self-hosted deployments where operational simplicity and resource efficiency matter more than elastic scaling.
...
| Dimension | Supabase (SQL) | DarshanDB (Triple/EAV) |
|-----------|----------------|------------------------|
...
**Verdict**: Supabase gives you the full power of relational modeling. If your data is naturally tabular and you need complex joins, aggregations, and window functions, SQL is unbeatable. DarshanDB's triple store trades query sophistication for schema flexibility and built-in time-travel -- it is better suited for applications where the data shape evolves rapidly (CMS, multi-tenant SaaS, collaborative tools) than for analytical workloads.

**Honest limitation**: The EAV model incurs a measurable performance penalty on multi-attribute filters compared to a properly indexed relational table. A query filtering on 5 attributes requires 5 self-joins on the triples table. DarshanDB mitigates this with plan caching and parallel query execution, but it will not outperform a single indexed scan on a dedicated column.
...
### DarshanDB: In-Process Broadcast with Diff Engine

DarshanDB's sync engine is compiled into the server binary:
...
**Verdict**: DarshanDB has significantly lower latency because there is no inter-process hop or WAL decode step. The diff-based approach also reduces bandwidth -- clients receive only the fields that changed, not entire row payloads. However, Supabase's Elixir-based Realtime server is battle-tested at scale with millions of concurrent connections. DarshanDB's Tokio-based approach is efficient but unproven at that level.
...
### DarshanDB: Built-In Auth

DarshanDB's auth engine is compiled into the server:
...
**Verdict**: Supabase has broader auth coverage, especially for enterprise use cases (SAML, phone OTP, 20+ OAuth providers, email templates). DarshanDB has stronger cryptographic defaults (Argon2id with OWASP params, RS256 by default, device-fingerprint binding) and lower latency (no inter-service token validation hop). If you need phone OTP or SAML, Supabase wins outright. If you need tight auth-to-data integration in a self-hosted environment, DarshanDB's in-process auth eliminates an entire class of token-relay bugs.
...
### DarshanDB: Composable Permission Rules

DarshanDB uses a JSON-configurable permission engine with composable rules:
...
| Dimension | Supabase RLS | DarshanDB Permissions |
|-----------|-------------|----------------------|
...
| **Bypass risk** | Zero -- even `psql` respects RLS (unless BYPASSRLS role) | Moderate -- direct Postgres access bypasses all DarshanDB permissions |
...
### DarshanDB: Server Functions (Node Subprocess)

```typescript
// darshan/functions/hello.ts
import { query, mutation, action } from "darshandb";
...
| Dimension | Supabase Edge Functions | DarshanDB Server Functions |
|-----------|------------------------|---------------------------|
...
**Verdict**: Supabase Edge Functions have stronger isolation (V8 sandbox) and are better suited for untrusted or multi-tenant code. DarshanDB Server Functions are tighter integrated with the data layer (direct `ctx.db` access, built-in scheduling, type-differentiated functions) but lack sandbox isolation -- they trust the function author. For a self-hosted system where you control the function code, DarshanDB's model is more productive. For a managed platform, Supabase's isolation model is mandatory.
...
**Verdict**: Supabase Storage is more feature-complete, especially on the managed platform (CDN image transforms, dashboard UI). DarshanDB Storage is more operationally simple (same binary) and offers more backend flexibility (local FS for development, any S3-compatible for production). The local FS backend is a significant advantage for self-hosted deployments that do not want to run MinIO alongside Postgres.
...
| Feature | Supabase CLI | DarshanDB CLI |
|---------|-------------|---------------|
| Project init | `supabase init` | `darshan init` |
| Local dev | `supabase start` (full Docker Compose stack) | `darshan dev` (single binary + Postgres) |
...
| Feature | Supabase Studio | DarshanDB |
|---------|----------------|-----------|
...
**Verdict**: Supabase has a vastly more mature developer experience. The JavaScript SDK alone has more usage than DarshanDB's entire user base. Supabase Studio is a genuine competitive advantage for onboarding non-backend developers. DarshanDB's developer experience is early-stage -- functional but missing polish, dashboard, and client SDK breadth. This is the single largest gap.
...
### DarshanDB Self-Hosted

```bash
# Install
curl -L https://github.com/darshjme/darshandb/releases/latest/download/darshandb-$(uname -s)-$(uname -m) -o darshandb
chmod +x darshandb

# Run
DATABASE_URL=postgres://user:pass@localhost:5432/darshandb ./darshandb
```

- **Container count**: 1 (the binary) + 1 (Postgres, probably already running)
- **Minimum RAM**: 30-80 MB for the DarshanDB process
- **Configuration**: Environment variables (12-factor). Key ones: `DATABASE_URL`, `DARSHAN_JWT_SECRET` or `DARSHAN_JWT_PRIVATE_KEY`/`DARSHAN_JWT_PUBLIC_KEY`, `DARSHAN_PORT`
...
- **Docker option**: Also available as a 2-container Docker Compose (Postgres + DarshanDB)

### Comparison

| Dimension | Supabase Self-Hosted | DarshanDB Self-Hosted |
|-----------|---------------------|----------------------|
...
**Verdict**: DarshanDB's self-hosting story is dramatically simpler. This is not a marginal difference -- it is an order of magnitude less operational complexity. For teams that do not want to run a Kubernetes cluster just for their backend-as-a-service, DarshanDB's single binary is a compelling proposition.
...
1. **Ecosystem maturity**: The JavaScript SDK has millions of downloads. Client libraries exist for every major platform. Community tutorials, templates, and integrations are abundant. DarshanDB is in its infancy here.

2. **Admin dashboard (Studio)**: A genuinely excellent web interface for managing tables, auth users, storage, and running SQL. DarshanDB has no dashboard.

3. **Managed hosting**: Supabase Cloud handles infrastructure, backups, monitoring, SSL, and scaling. DarshanDB is self-hosted only.

4. **SQL power**: If you need complex analytical queries, window functions, CTEs, or lateral joins, Supabase gives you raw PostgreSQL. DarshanDB's DarshanQL covers common cases but cannot match SQL's expressiveness.

5. **Auth provider breadth**: 20+ OAuth providers, phone OTP, SAML SSO. DarshanDB has 4 OAuth providers and no phone/SAML support.

6. **Database extensions**: Supabase exposes the full PostgreSQL extension ecosystem (PostGIS, pg_cron, pg_stat_statements, pgvector, etc.). DarshanDB uses Postgres as a backing store but abstracts over it -- most extensions are not directly accessible through the DarshanQL interface.

7. **Edge Functions isolation**: V8 isolates provide genuine sandboxing. DarshanDB's Node subprocesses rely on OS-level controls, which are weaker.

8. **Community and funding**: Supabase has raised $116M+ in venture capital, employs 100+ engineers, and has a large open-source community. DarshanDB is a solo/small-team project.

9. **PostgreSQL ecosystem**: If your team already knows SQL, Supabase has zero learning curve for the data layer. DarshanDB requires learning the triple store model and DarshanQL.

10. **Horizontal scaling**: Supabase can scale individual services (add more Realtime nodes, more PostgREST instances). DarshanDB scales vertically only.

---

## 11. What DarshanDB Does Better

Being honest about where DarshanDB is ahead:
...
6. **Built-in vector search**: pgvector integration with automatic embedding pipeline. Configure `DARSHAN_EMBEDDING_PROVIDER=openai` and attributes auto-embed on write. Supabase supports pgvector but requires manual embedding and query construction.
...
## 12. Migration Path: Supabase to DarshanDB
...
- Export Supabase `auth.users` table (password hashes are Bcrypt -- DarshanDB uses Argon2id)
...
- OAuth tokens: Re-authenticate through DarshanDB's OAuth flow (tokens are not portable)

### Phase 3: Permission Migration

Convert RLS policies to DarshanDB permission rules:
...
Note: Complex RLS policies using SQL functions or subqueries may not translate directly to DarshanDB's permission DSL. These require custom handler logic.
...
// After (DarshanDB)
const { data } = await darshan.query({
  from: 'Post',
...
| **Auth gap** | Medium | If using phone OTP or SAML, these are not available in DarshanDB. Find alternative providers or defer migration. |
| **SDK maturity** | High | DarshanDB's client SDKs are early-stage. Budget time for SDK issues and workarounds. |
...
| Dimension | Choose Supabase When | Choose DarshanDB When |
|-----------|---------------------|----------------------|
...
DarshanDB is not a drop-in Supabase replacement. It is a different architectural bet: that a single, efficient binary with a flexible data model can serve the 80% of use cases that do not need the full power of a managed Postgres platform. For the 20% that do, Supabase remains the better choice.
