...
DarshanDB is an alpha-stage Rust binary that bets on a different architecture: a triple store over PostgreSQL, a single process instead of a container fleet, and real-time by default rather than bolted on. It is not production-ready. It is worth understanding why it exists.

| Dimension | Appwrite | DarshanDB |
|-----------|----------|-----------|
...
### DarshanDB: Single Binary

DarshanDB is one Rust binary linked against Tokio (async runtime) and Axum (HTTP framework). Everything runs in a single process:

```
darshandb-server          # HTTP + WebSocket + auth + query engine + sync
postgres                  # External dependency
```

Docker Compose for DarshanDB has two containers: the server and Postgres. Memory footprint idle is approximately 40-80 MB for the server process plus whatever Postgres needs. The entire system runs on a 1 GB VPS.

The trade-off is clear. Appwrite's architecture gives you service isolation and mature operational patterns. DarshanDB's architecture gives you simplicity and resource efficiency. A single process means a single point of failure but also a single thing to monitor, deploy, and debug.

**What this means in practice:**

| Concern | Appwrite | DarshanDB |
|---------|----------|-----------|
...
### DarshanDB: Triple Store over PostgreSQL

DarshanDB stores everything as (entity_id, attribute, value) triples in a single PostgreSQL table. There are no collections in the traditional sense. A "collection" is just a query filter on entity type. There are no schemas to define before writing data.

```sql
-- What Appwrite stores as a document:
-- { "name": "Alice", "email": "alice@example.com", "role": "admin" }

-- DarshanDB stores as triples:
-- (e_01, "name",  "Alice")
...
DarshanDB's real-time is in-process. The sync engine uses `tokio::broadcast` channels (no Redis hop). When a mutation occurs in the triple store, a `ChangeEvent` is emitted containing the affected entity IDs and attributes. The broadcaster:
...
The trade-off: DarshanDB's approach re-executes queries on every mutation that touches relevant entities. At high write throughput with many active subscriptions, this could become a bottleneck. Appwrite's approach is simpler (publish full document) but scales more predictably because it does not re-run queries.

**Comparison:**

| Aspect | Appwrite | DarshanDB |
|--------|----------|-----------|
...
DarshanDB's auth covers the fundamentals:
...
**The honest take:** Appwrite's auth is production-grade. DarshanDB's auth has strong cryptographic foundations (better password hashing, better JWT defaults) but lacks the breadth of flows that real applications need. A startup building a B2B SaaS would hit DarshanDB's auth limitations within weeks.
...
DarshanDB's function system exists at the architecture level but is not production-ready. The code shows:
...
- **@darshandb/client-core**: Framework-agnostic TypeScript core (HTTP client, WebSocket client, auth state, query builder).
- **@darshandb/react**: `useQuery`, `useMutation`, `useAuth`, `usePresence`, `useStorage` hooks. Follows React conventions (Suspense-compatible, automatic refetch on focus).
- **@darshandb/angular**: Signals + RxJS observables via `DarshanService`. Angular-native DI and change detection.
- **@darshandb/nextjs**: Server Components support, App Router + Pages Router, server-side data fetching with client hydration.
- **darshandb-php**: Composer package with Laravel service provider, Blade directives, Eloquent-like query builder.
- **darshandb-python**: Sync + async client, FastAPI dependency injection, Django integration.
...
```bash
# Option 1: Binary
DATABASE_URL=postgres://user:pass@localhost:5432/darshandb \
  ./darshandb-server
...
DarshanDB needs Postgres. That is the only external dependency. The binary is statically linked Rust. The Docker Compose file has two services: the server and `pgvector/pgvector:pg16`. Total RAM footprint: 256 MB is comfortable.

What DarshanDB does NOT provide yet:
...
**The honest take:** Appwrite is harder to run but gives you everything out of the box. DarshanDB is trivial to start but leaves TLS, reverse proxy, backups, and monitoring to you. For a $5 VPS running a side project, DarshanDB's simplicity wins. For a production SaaS with a team, Appwrite's completeness wins.

---

## 8. What Appwrite Does Better

These are areas where Appwrite is objectively ahead and DarshanDB has no near-term path to matching.

### Maturity and Stability

Appwrite has been in development since 2019, is backed by venture funding, has a full-time team, and is used in production by thousands of projects. DarshanDB has 731 tests and zero production deployments. This gap does not close with clever architecture.

### SDK Ecosystem

14+ SDKs including mobile (Flutter, Android, iOS, React Native) versus 6 web/server SDKs. Mobile SDKs are essential for many projects. DarshanDB has no mobile story.

### Messaging

Appwrite 1.5+ includes built-in messaging: email (SMTP, Mailgun, SendGrid), SMS (Twilio, Vonage, Textmagic), and push notifications (FCM, APNS). DarshanDB has no messaging capability. Building this from scratch is months of work.

### Teams and Organizations

Appwrite has first-class team management: create teams, invite members, assign roles, team-level permissions on resources. DarshanDB has user-level permissions but no team/organization abstraction. For B2B SaaS, this is a hard requirement.

### Console UI

Appwrite's web console is polished, mature, and covers all operations: database management, user management, function deployment, storage browsing, real-time monitoring. DarshanDB's admin dashboard exists but covers basic data viewing only.

### Geographic Deployment

Appwrite Cloud offers multiple regions. Self-hosted Appwrite can run anywhere Docker runs. DarshanDB has no horizontal scaling, no multi-region support, and no managed cloud offering.

### Community and Documentation

Appwrite has 40k+ GitHub stars, active Discord, comprehensive docs, video tutorials, and a marketplace. DarshanDB has documentation but no community yet.

---

## 9. What DarshanDB Does Better

These are areas where DarshanDB's architecture provides genuine advantages, not just theoretical ones.

### Resource Efficiency

A DarshanDB instance (server + Postgres) runs comfortably in 256 MB RAM. An Appwrite instance needs 1.5-3 GB minimum. For developers in regions where cloud costs matter (India, Southeast Asia, Africa, South America), running on a $5 VPS versus a $20 VPS is the difference between building and not building.
...
Appwrite uses MariaDB. It works, but it limits what the database layer can do. DarshanDB uses PostgreSQL, which means:
...
DarshanDB pushes field-level diffs instead of full documents. For a 50-field entity where one field changes, DarshanDB sends the one changed field. Appwrite sends all 50 fields. Over a WebSocket connection with hundreds of subscriptions and frequent updates, this difference is significant.

DarshanDB also includes built-in presence tracking (who is online in a room) and pub/sub (custom event channels), which Appwrite does not provide natively.

### Embedding Pipeline

DarshanDB has a built-in auto-embedding pipeline. When text triples are written, embeddings are generated via OpenAI or Ollama and stored in pgvector. This enables semantic search (find similar items), hybrid search (combine text + vector ranking via Reciprocal Rank Fusion), and RAG patterns without external infrastructure.
...
DarshanDB does not have published benchmarks yet. The performance advantage is architectural, not proven.
...
- You need horizontal scaling across regions (Appwrite supports it in theory, DarshanDB does not)
...
| Criterion | Weight | Appwrite | DarshanDB | Notes |
|-----------|--------|----------|-----------|-------|
| Production readiness | 10 | 9/10 | 3/10 | Appwrite is battle-tested. DarshanDB is alpha. |
...
| SDK ecosystem | 7 | 9/10 | 5/10 | 14 vs 6. No mobile for DarshanDB. |
...
DarshanDB is alpha software built by one developer. Appwrite is a well-funded company with a full engineering team, years of production hardening, and a large community. Any comparison that declares DarshanDB "better" in aggregate would be dishonest.

What DarshanDB has is a different architectural thesis: that a single Rust binary with a triple store over PostgreSQL, diff-based real-time, and built-in vector search is a better foundation for the next decade of application development. Whether that thesis holds depends on execution, community, and time.

Appwrite is the practical choice today. DarshanDB is an experiment worth watching.

---

*This comparison was written from reading both codebases. Appwrite's source is at [github.com/appwrite/appwrite](https://github.com/appwrite/appwrite). DarshanDB's source is at [github.com/darshjme/darshandb](https://github.com/darshjme/darshandb). If any claims are inaccurate, please open an issue.*
