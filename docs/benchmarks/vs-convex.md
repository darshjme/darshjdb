# DarshanDB vs Convex: Technical Comparison
...
Convex is a cloud-native reactive database with excellent TypeScript DX, deterministic server functions, and automatic query reactivity. DarshanDB is a self-hosted BaaS built on a triple store (EAV) data model with multi-language SDK support. They share similar goals -- making real-time backends easy -- but take fundamentally different architectural approaches.

**Convex's strengths:** Developer experience, type safety, deterministic execution, battle-tested reactivity.
**DarshanDB's strengths:** Self-hosting from day one, data model flexibility, multi-language SDKs, no vendor lock-in.

Neither is universally better. This document is written by the DarshanDB team and we will be honest about where Convex is ahead.
...
DarshanDB stores everything as `(entity_id, attribute, value)` triples in a single Postgres table. The schema is inferred from data rather than declared upfront. Entities are grouped by a `:db/type` attribute. Values are JSONB with a type discriminator (string, integer, float, boolean, null, reference, json).
...
### DarshanDB: Change Event + Dependency Tracking + Diff Push

DarshanDB uses a three-stage pipeline (implemented in `packages/server/src/sync/` and `packages/server/src/query/reactive.rs`):
...
```rust
// DarshanDB: explicit dependency extraction
...
**Honest assessment:** Convex's approach is simpler and more reliable. Re-executing the whole query eliminates an entire class of bugs around stale caches and diff correctness. DarshanDB's diff-based approach is theoretically more bandwidth-efficient, especially for large result sets, but:
...
2. Convex has years of production usage validating their reactivity model. DarshanDB has zero production deployments.

**Advantage: Convex, decisively.** DarshanDB's architecture has better theoretical efficiency, but Convex ships a working product.
...
### DarshanDB: Subprocess-Based Execution

DarshanDB defines a `RuntimeBackend` trait with a `ProcessRuntime` implementation that spawns Deno or Node subprocesses per invocation. Functions are categorized into queries, mutations, and actions (mirroring Convex's taxonomy). Resource isolation is via OS-level controls (CPU time limit, memory limit, concurrency semaphore).

```rust
// DarshanDB: pluggable runtime backend
...
**Honest assessment:** Convex's deterministic V8 runtime is a genuine technical achievement. The determinism guarantee makes caching and reactivity reliable by construction. DarshanDB's subprocess approach is a pragmatic starting point, but the per-invocation spawn cost makes it unsuitable for high-throughput workloads. The codebase explicitly notes "subprocess-based, not V8" and the function runtime is listed as not working in CODE.md.

**Advantage: Convex, significantly.** DarshanDB has the right abstractions (the `RuntimeBackend` trait allows swapping in an embedded V8 later), but today it is a subprocess placeholder versus Convex's production-grade isolate system.
...
DarshanDB transactions are backed by Postgres. The `set_triples()` method writes a batch of triples under a single transaction ID obtained from a Postgres sequence (`darshan_tx_seq`). Retraction (soft delete) sets `retracted = true` rather than physically deleting rows.
...
**Honest assessment:** Convex transactions are more powerful because mutation functions can read and write within the same serializable transaction. DarshanDB's transactions are write-only batches -- you cannot read a value, make a decision, and write based on it within a single atomic operation. This is a meaningful limitation for patterns like "transfer funds if balance sufficient."

The Postgres foundation gives DarshanDB a solid base to build on (Postgres serializable isolation is available), but the current implementation does not expose transactional reads to function code.
...
DarshanDB schemas are inferred from data at runtime via `get_schema()`, which scans triple data to discover entity types, their attributes, value types, and reference relationships. The TypeScript SDK uses generic type parameters for queries:

```typescript
// DarshanDB: generic type parameter
...
**Honest assessment:** Convex's type safety story is best-in-class among BaaS platforms. DarshanDB's approach of generic type parameters provides some safety but relies on developers keeping types in sync manually. The triple store's string-based attribute names (`"email"`, `"name"`) mean typos become runtime errors, not compile-time errors.
...
DarshanDB was designed for self-hosting from the start. The server is a single Rust binary that connects to Postgres. Docker Compose files are provided for both development and production. The operational requirements are minimal: Postgres + the DarshanDB binary.

```yaml
# DarshanDB docker-compose.yml -- that's the whole deployment
services:
  postgres:
    image: postgres:16
  darshandb:
    build: .
    environment:
      DATABASE_URL: postgres://darshan:darshan@postgres:5432/darshandb
```
...
**Honest assessment:** Self-hosting is genuinely where DarshanDB has a structural advantage. Building on Postgres means operators can use existing Postgres expertise, tooling, and infrastructure. Convex's custom storage engine means self-hosting it requires learning Convex-specific operations.

That said, DarshanDB's self-hosting advantage is somewhat offset by the fact that many features listed in the codebase are not yet wired end-to-end (real-time, functions, auth enforcement). Self-hosting a feature-incomplete system is less valuable than using Convex's fully working managed service.

**Advantage: DarshanDB architecturally, Convex practically.** DarshanDB wins on the self-hosting design; Convex wins on having a working product to host.
...
- CLI with `darshan dev` (compiles but does not auto-start Postgres)
...
**Honest assessment:** Convex has one of the best developer experiences of any backend platform. Their investment in DX -- from error messages to documentation to the dashboard -- is evident. DarshanDB has broader language coverage (Python, PHP, Angular SDKs) but the DX of each individual SDK is less polished. The admin dashboard rendering mock data is a significant gap.

**Advantage: Convex, substantially.** DarshanDB's breadth (multi-language, multi-framework) is notable, but Convex's depth in TypeScript DX is hard to match.
...
1. **Reactivity that works today.** Convex's reactive queries are production-tested. DarshanDB's sync pipeline exists as separate components that are not connected.

2. **Deterministic function execution.** Patching `Date.now()` and `Math.random()` for deterministic caching is a clever engineering decision that DarshanDB has no equivalent for.

3. **End-to-end type safety.** Schema-to-client type inference eliminates an entire category of bugs. DarshanDB's string-based attribute names are a step backward.

4. **Transactional reads + writes.** Being able to read, decide, and write within one serializable transaction is fundamental. DarshanDB only batches writes.

5. **Onboarding experience.** `npx convex dev` to a working reactive backend in under 60 seconds. DarshanDB requires Postgres, environment variables, and understanding the triple store model.

6. **Battle-tested at scale.** Convex serves production traffic for real applications. DarshanDB has zero production deployments and explicitly states it is "not production-ready."

7. **Automatic conflict resolution.** OCC with automatic retry means developers never think about write conflicts. DarshanDB leaves this to Postgres defaults.

---

## 9. What DarshanDB Does Better

Where DarshanDB has genuine advantages:
...
4. **Multi-language SDKs.** Python, PHP, Angular, and Next.js SDKs alongside the TypeScript client. Convex is TypeScript-first and while there are community SDKs for other languages, they are not first-party. DarshanDB's PHP/Laravel and Python SDKs are first-party with 52 and 141 tests respectively.
...
6. **No vendor lock-in.** Data is in Postgres tables. If DarshanDB disappears tomorrow, your data is still queryable with standard SQL. Convex data export requires their tooling.
...
The `ConvexCompat` class wraps a `DarshanDB` client instance and exposes Convex-style methods:
...
The compat layer translates Convex-style filter objects to DarshanDB `where()` calls:
...
- Uses the real DarshanDB transaction API (not a parallel implementation)
...
**Gaps:**
- **No index hints.** Convex's `.withIndex("by_channel", q => q.eq("channel", id))` has no equivalent. DarshanDB relies on Postgres query planning.
...
The script reads exported Convex data (directory of JSON files or a single combined JSON file) and imports it into DarshanDB via the REST API.
...
3. Convert each Convex document to a DarshanDB mutation op:
...
**Gaps:**
- **No reference resolution.** Convex document IDs like `j571hp2a5p6ej7vhq7bwfq1mzh6g2pxv` are used as-is. If DarshanDB expects UUID format for references, cross-document references will break. The script does not detect or convert reference fields.
- **No schema migration.** Convex schemas (validators, indexes) are not translated into DarshanDB equivalents. Only data is migrated.
...
**Verdict:** The script handles the straightforward case well: flat Convex documents with primitive fields migrated to DarshanDB triples. It falls short on reference resolution, schema translation, and operational safety (no idempotency, no rollback). A production migration would need additional tooling around this script.
...
| Dimension | Convex | DarshanDB | Winner |
|-----------|--------|-----------|--------|
...
| Self-hosting | Recently available | Day-one design | DarshanDB |
| Vendor lock-in | Medium | Low (Postgres) | DarshanDB |
| Multi-language | TypeScript-primary | TS, Python, PHP, Angular | DarshanDB |
| Audit/history | Not built-in | Append-only + point-in-time | DarshanDB |
| Production readiness | Yes | No (alpha) | Convex |
| DX polish | Excellent | Work in progress | Convex |
| Bandwidth efficiency | Full re-send | Delta diffs (when working) | DarshanDB (theoretical) |
| Security depth | Managed + auth providers | 12 CVEs found and fixed, MFA, RLS | DarshanDB |
...
This comparison is written honestly. Convex is ahead on most dimensions that matter for shipping products today. DarshanDB's architectural choices -- the triple store, append-only writes, diff-based reactivity, multi-language SDKs -- represent bets on flexibility and self-hosting that may pay off as the project matures. But "may pay off" is not the same as "works today."

The ConvexCompat layer and migration script demonstrate intent to provide a path for Convex users, but both need work before they can support non-trivial migrations. Teams considering DarshanDB should evaluate the current state of the code (CODE.md is unflinchingly honest about what works and what does not) rather than the roadmap.
